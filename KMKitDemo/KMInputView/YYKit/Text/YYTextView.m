...
- (BOOL)_isPasteboardContainsValidValue {
    ...
    if (pasteboard.URL) {
        if ([pasteboard.URL absoluteString].length > 0) {
            return YES;
        }
    }
...
- (void)setText:(NSString *)text {
    ...
    //清除缓存YYTextHighlight 等 attribute
    [_innerText removeAttribute:YYTextHighlightAttributeName range:_innerText.rangeOfAll];
    [_innerText removeAttribute:NSForegroundColorAttributeName range:_innerText.rangeOfAll];
...
- (void)paste:(id)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(messageViewPaste)]) {
        self.isPaste = YES;
        [self.delegate messageViewPaste];
