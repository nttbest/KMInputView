...
@class KMSecurityPolicy;
@class KMBaseAPI;
@protocol KMAPI;
@protocol KMRPCProtocol;

NS_ASSUME_NONNULL_BEGIN

#pragma mark -
@protocol KMMultipartFormData
...
@protocol KMHttpHeaderDelegate <NSObject>
 ...
 *  用于组织POST体的block
 */
@property (nonatomic, copy, nullable) void (^apiRequestConstructingBodyBlock)(id<KMMultipartFormData> _Nonnull formData);
 ...
 */
- (nullable id<KMRPCProtocol>)rpcDelegate;

/**
 *  HTTPHeader Field Delegate
 */
- (nullable id<KMHttpHeaderDelegate>)apiHttpHeaderDelegate;
 ...
 *  请求的类型:GET, POST
 *  @default
 *   KMRequestMethodTypePost
 *
 *  @return KMRequestMethodType
 */
- (KMRequestMethodType)apiRequestMethodType;

/**
 *  Request 序列化类型：JSON, HTTP, 见KMRequestSerializerType
 *  @default
 *   KMResponseSerializerTypeJSON
 *
 *  @return KMRequestSerializerTYPE
 */
- (KMRequestSerializerType)apiRequestSerializerType;

/**
 *  Response 序列化类型： JSON, HTTP
 *
 *  @return KMResponseSerializerType
 */
- (KMResponseSerializerType)apiResponseSerializerType;
 ...
 *  @return HTTPS证书验证策略
 */
- (nonnull KMSecurityPolicy *)apiSecurityPolicy;
